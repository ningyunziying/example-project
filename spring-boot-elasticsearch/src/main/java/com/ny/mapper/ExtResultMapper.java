...
public class ExtResultMapper extends AbstractResultMapper {
    ...
    private static <T> boolean isListField(T result, String fieldName) throws IllegalAccessException, NoSuchMethodException, InvocationTargetException, NoSuchFieldException {
        Class clazz = PropertyUtils.getPropertyType(result, fieldName);
        if (null != clazz) {
            String className = clazz.getName();
            if (className.equalsIgnoreCase("java.util.List")) {
                return true;
            }
        }
        return false;
    ...
    private String concat(Text[] texts, String breaker) {
        StringBuffer sb = new StringBuffer();
        for (Text text : texts) {
            sb.append(text.toString());
            sb.append(breaker);
    ...
    private List<String> frgToStringList(Text[] texts) {
